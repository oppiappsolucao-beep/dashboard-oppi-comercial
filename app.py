...
def render_scoring_page(df: pd.DataFrame, columns: dict) -> None:
    ...
    install_custom_sidebar_reopen_button()
...
# =========================================================
# CORREÇÃO FINAL REAL: BOTÃO CUSTOMIZADO PARA REABRIR O MENU
# =========================================================
def install_custom_sidebar_reopen_button() -> None:
    """
    Substitui visualmente a seta nativa cortada do Streamlit por um botão próprio,
    cinza e posicionado dentro da faixa livre acima de Empresas. O botão aparece
    somente quando o menu lateral está recolhido e aciona o controle nativo.
    """
    components.html(
        """
        <script>
            (function () {
                ...
                const hostDocument = getHostDocument();
                const hostWindow = window.parent || window;
                const BUTTON_ID = 'oppi-custom-sidebar-reopen';
                ...
                function nativeCandidates() {
                    ...
                    fixedSelectors.forEach(function (selector) {
                        hostDocument.querySelectorAll(selector).forEach(function (element) {
                            if (element && element.id !== BUTTON_ID && !candidates.includes(element)) {
                                candidates.push(element);
                            }
                        });
                    });

                    /* Compatibilidade com versões do Streamlit que não expõem data-testid. */
                    hostDocument.querySelectorAll('button').forEach(function (button) {
                        if (!button || button.id === BUTTON_ID || candidates.includes(button)) return;
                        const rect = button.getBoundingClientRect();
                        const aria = (button.getAttribute('aria-label') || '').toLowerCase();
                        const title = (button.getAttribute('title') || '').toLowerCase();
                        const closeToUpperLeft = rect.left < 95 && rect.top < 135 && rect.width <= 60 && rect.height <= 60;
                        const looksLikeSidebar = aria.includes('sidebar') || title.includes('sidebar') || button.querySelector('svg');
                        if (closeToUpperLeft && looksLikeSidebar) candidates.push(button);
                    });
                ...
                function ensureButton() {
                    let button = hostDocument.getElementById(BUTTON_ID);
                    ...
                    button.setAttribute('title', 'Abrir menu lateral');
                    button.innerHTML = '&#8250;';
                    ...
                    button.addEventListener('mouseenter', function () {
                        ...
                        button.style.boxShadow = '0 10px 22px rgba(0,0,0,0.20)';
                    });
                    ...
                    button.addEventListener('click', function () {
                        ...
                        const target = candidates.find(function (element) {
                            return element !== button && typeof element.click === 'function';
                        });
                        if (target) target.click();
                        hostWindow.setTimeout(sync, 120);
                        hostWindow.setTimeout(sync, 420);
                    });
                ...
                function hideNativeCollapsedArrow() {
                    nativeCandidates().forEach(function (element) {
                ...
                function sync() {
                    const customButton = ensureButton();
                    ...
                    hideNativeCollapsedArrow();
                    customButton.style.display = 'flex';
                    customButton.style.top = '34px';
                    customButton.style.left = '18px';
                }

                sync();
                hostWindow.setTimeout(sync, 80);
                hostWindow.setTimeout(sync, 250);
                hostWindow.setTimeout(sync, 700);
                hostWindow.setInterval(sync, 500);
            })();
        </script>
        """,
        height=0,
